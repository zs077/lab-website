...
export const publications: Publication[] = [
  ...
  },
  {
    ...
    venue: 'IEEE Transactions onIntelligent Transportation Systems 15(6), 2551-2560, 2014.',
    year: 2014,
    category: 'journal',
    abstract: {
  ...
  {
    id: 9,
    title: 'An integrated hazard identification method based on the hierarchical Colored Petri Net.',
    authors: 'Rui Wang, Wei Zheng, Ci Liang,Tao Tang',
    venue: 'Safety science 88, 166-179, 2016.',
    year: 2016,
    category: 'journal',
    abstract: {
      ...
      zh: '传统的社会技术系统危害分析方法无法涵盖复杂的组织结构、系统与人行为之间的交互作用、子系统间的相互关联因素以及特定社会的安全文化。本文提出了一种名为基于形式化模型的系统理论过程分析（BFM-STPA）的集成危害识别方法，可以解决上述问题。首先，利用着色 Petri 网（CPN）构建社会技术系统的分层控制结构模型，CPN 具有强大的描述能力和可执行性，也可被视为图形化的系统规范。其次，根据一系列指导条件，基于 CPN 模型识别危害。第三，通过追踪 CPN 模型生成的可达图中的状态变化，找出导致危害的综合因素。最后，生成集成危害日志，用于进一步的危害分析和安全导向设计。本文将上述方法应用于中国列车控制系统 3 级（CTCS-3）。针对临时限速场景，详细阐述了风险识别过程。与危险与可操作性分析（HAZOP）生成的风险日志相比，BFM-STPA 生成的风险日志不仅涵盖了子系统故障，还涵盖了子系统间交互偏离设计意图、人为错误以及与 CTCS-3 临时限速场景相关的社会技术缺陷。',
    },
  },
  {
    id: 10,
  ...
  {
    id: 14,
    title: 'Uncertainty quantification for semantic segmentation models via evidential reasoning.',
    authors: 'Rui Wang, Mengying Wang, Ci Liang, Zhouxian Jiang',
    venue: '42nd International Conference on ComputerSafety, Reliability and Security (SafeComp2023), Toulouse, France, September, 2023.',
    year: 2023,
    category: 'conference',
    abstract: {
      en: 'Deep learning models typically render decisions based on probabilistic outputs. However, in safety-critical applications such as environment perception for autonomous vehicles, erroneous decisions made by semantic segmentation models may lead to catastrophic results. Consequently, it would be beneficial if these models could explicitly indicate the reliability of their predictions. Essentially, stakeholders anticipate that deep learning models will convey the degree of uncertainty associated with their decisions. In this paper, we introduce EviSeg, a predictive uncertainty quantification method for semantic segmentation models, based on Dempster-Shafer (DS) theory. Specifically, we extract the discriminative information, i.e., the parameters and the output features from the last convolution layer of a semantic segmentation model. Subsequently, we model this multi-source evidence to the evidential weights, thereby estimating the predictive uncertainty of the semantic segmentation model with the Dempster’s rule of combination. Our proposed method does not require any changes to the model architecture, training process, or loss function. Thus, this uncertainty quantification process does not compromise the model performance. Validated on the urban road scene dataset CamVid, the proposed method enhanced computational efficiency by three to four times compared to the baseline method, while maintaining comparable performance with baseline methods. This improvement is critical for real-time applications.',
      zh: '深度学习模型通常基于概率输出进行决策。然而，在诸如自动驾驶车辆环境感知等安全关键型应用中，语义分割模型的错误决策可能导致灾难性后果。因此，如果这些模型能够明确指出其预测的可靠性，将大有裨益。本质上，利益相关者期望深度学习模型能够传达其决策相关的不确定性程度。本文提出了一种基于 Dempster-Shafer (DS) 理论的语义分割模型预测不确定性量化方法——EviSeg。具体而言，我们从语义分割模型的最后一个卷积层提取判别信息，即参数和输出特征。随后，我们将这些多源证据建模为证据权重，从而利用 Dempster 组合规则估计语义分割模型的预测不确定性。我们提出的方法无需对模型架构、训练过程或损失函数进行任何更改。因此，这种不确定性量化过程不会影响模型性能。在城市道路场景数据集 CamVid 上验证，所提出的方法与基线方法相比，计算效率提高了三到四倍，同时保持了与基线方法相当的性能。这一改进对于实时应用至关重要。',
    },
  },
  {
    id: 15,
